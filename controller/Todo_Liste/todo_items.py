...
def edit_todo_items():
    ...
    item_to_edit = session.query(Todoitem).filter(
        Todoitem.item_ID == item_id).first()

    if request.method == "POST":
        if edit_item_form.validate_on_submit():
            item_to_edit = item_to_edit = session.query(
                Todoitem).filter(Todoitem.item_ID == item_id).first()
